...
write_map <- function(x, path, name)
{
  
  raster::writeRaster(x, file.path(path,paste0(name,".tif")))
  gdalUtilities::gdal_translate(src_dataset =  file.path(path,paste0(name,".tif")),
                                dst_dataset =  file.path(path,paste0(name,".map")), of =  "PCRaster")
  unlink(file.path(path,paste0(name,".tif")))
}
```
...
raster::plot(dgm)
raster::writeRaster(dgm, filename = "models/E3D/dgm.tif")

gdal_translate(src_dataset =  "models/E3D/dgm.tif",
               dst_dataset =  "models/E3D/dgm.map", of =  "PCRaster")

write_map(dgm, "models/E3D", "dgm_lis")
```
...
'-x -r "D:\\Projekte\\E4D\\paper_erosion_modeling\\models\\Lisem\\Lisem_skript\\rainfall_200721_new.script" -d "D:\\Projekte\\E4D\\paper_erosion_modeling\\models\\Lisem\\maps_05\\"', stdout = FALSE
)
...
```{r}
setwd("D:\\Projekte\\E4D\\paper_erosion_modeling\\models\\Lisem\\maps_05\\")
...
for(z in names(velx))
{
  raster::plot(velx[[z]], zlim = c(-0.5,0.5), main = z)
}
```
